...
const ChatWindow: React.FC<ChatWindowProps> = ({
  ...
  onOpenSettings, onRoomRenamed, onLeave, showToast,
}) => {
  ...
  const revealViewOnce = useCallback((eid: string, pt: Record<string, unknown>) => {
    setViewedOnceIds(p => { const n = new Set(p); n.add(eid); try { localStorage.setItem('frame-viewed-once', JSON.stringify([...n])); } catch {} return n; });
    const dur = getViewOnceDuration(pt);
    const timer = setTimeout(() => { setHiddenOnceIds(p => { const n = new Set(p); n.add(eid); try { localStorage.setItem('frame-hidden-once', JSON.stringify([...n])); } catch {} return n; }); setMessages(p => p.map(m => m.event.eventId === eid ? { ...m, plaintext: null, decryptionError: 'View-once message already viewed' } : m)); deleteMessage(eid).catch(e => console.error('[ViewOnce] Delete failed:', e)); delete viewOnceTimersRef.current[eid]; }, dur);
  ...
  const handleTextareaChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => { setInputValue(e.target.value); const ta = e.target; ta.style.height = 'auto'; ta.style.height = `${Math.min(ta.scrollHeight, 116)}px`; if (getSendTypingIndicators()) { const now = Date.now(); if (e.target.value.trim() && now - lastTypingSentRef.current > 3000) { lastTypingSentRef.current = now; setTyping(roomId, true).catch(() => undefined); } if (typingTimeoutRef.current) clearTimeout(typingTimeoutRef.current); if (e.target.value.trim()) typingTimeoutRef.current = setTimeout(() => setTyping(roomId, false).catch(() => undefined), 3000); else setTyping(roomId, false).catch(() => undefined); } };

  // Typing polling
  useEffect(() => { const p = async () => { try { const r = await getTypingUsers(roomId); setTypingUsers(r.typingUserIds); } catch {} }; void p(); const i = setInterval(() => void p(), 2000); return () => { clearInterval(i); setTypingUsers([]); }; }, [roomId]);
  ...
  // Forwarding
  const handleForwardMessage = useCallback(async (eid: string) => { setContextMenuEventId(null); setContextMenuPos(null); setForwardEventId(eid); try { const rooms = await listRooms(); setForwardRooms(rooms.filter(r => r.roomId !== roomId)); setShowForwardDialog(true); } catch (e) { console.error('Forward load failed:', e); } }, [roomId]);
  const handleForwardToRoom = useCallback(async (tid: string, tn: string) => { if (!forwardEventId) return; const m = messages.find(m => m.event.eventId === forwardEventId); if (!m?.plaintext) return; const body = typeof m.plaintext.body === 'string' ? m.plaintext.body : JSON.stringify(m.plaintext); try { const tm = await getRoomMembers(tid); const enc = await encryptForRoom(tid, 'm.room.message', { msgtype: 'm.text', body }, tm.map(m => m.userId)); await sendMessage(tid, 'm.room.encrypted', enc); showToast?.('success', `Forwarded to ${tn}`); } catch { showToast?.('error', 'Forward failed'); } setShowForwardDialog(false); setForwardEventId(null); }, [forwardEventId, messages, showToast]);
  ...
  const handleDeleteMessage = async (eid: string) => { setContextMenuEventId(null); setContextMenuPos(null); try { await deleteMessage(eid); setDeletedEventIds(p => new Set(p).add(eid)); } catch (e) { console.error('Delete failed:', e); } };
  ...
  const handleConsumedOnce = useCallback((eid: string) => { setConsumedOnceIds(p => { const n = new Set(p); n.add(eid); try { localStorage.setItem('frame-consumed-once', JSON.stringify([...n])); } catch {} return n; }); }, []);
