...
export async function processSyncResponse(
  ...
  try {
    ...
    // Extract to-device events from sync data if present
    const syncObj = syncData as unknown as Record<string, unknown>;
    const toDeviceEvents = syncObj['to_device'] ?? [];
    const deviceListChanges = syncObj['device_lists'] ?? {};
    const deviceOTKCounts = syncObj['device_one_time_keys_count'] ?? {};
    const unusedFallbackKeys =
      syncObj['device_unused_fallback_key_types'] as
        | Set<string>
        | undefined;
    ...
    await machine.receiveSyncChanges(
      ...
      new sdk.DeviceLists(),
      new Map(Object.entries(deviceOTKCounts as Record<string, number>)),
      unusedFallbackKeys,
    );
